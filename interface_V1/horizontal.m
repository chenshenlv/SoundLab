...
for i = 1 : length(filelist)
    ...
    load(sprintf('%s/%s',path,filelist(i).name));
%     hrtf_struct(i).data = data;
%     hrtf_struct(i).specs = specs;
%     hrtf_struct(i).num = num;
%     hrtf_struct(i).name = filelist(i).name;
...
for i=1:length(elevation_array)
    for j=1:length(azimuth_array)
        HRTF(k).el=elevation_array(i);
        HRTF(k).az=azimuth_array(j);
        HRTF(k).hrir_r=right(:,k)';
        HRTF(k).hrir_l=left(:,k)';
        HRTF(k).srate=srate;
        k=k+1;
    end
end
handles.hrtf_struct= HRTF;
% ind = []; % indices for horizontal plane
% for i = 1 : length(hrtf_struct)
%     data = hrtf_struct(i).data;
%     for j = 1 : length(data)
%         if data(j).elevation == 0 && mod(data(j).azimuth,15)==0
...
for i=1:12
    azs=handles.hrtf_struct(handles.ind(handles.R_azs_num(i))).az;
...
handles.fs = handles.hrtf_struct(1).srate;
set(handles.text_fs,'String',handles.fs)
...
infraBase = mk_pink(ceil(handles.fs*handles.durBase)); 
handles.fullSig = [];
for iRep = 1:handles.sigRepPrep
    handles.fullSig = [handles.fullSig, infraBase];
end;
mono = handles.fullSig;
handles.mono = mono;

save('infrapitch.mat', 'mono') 
...
azs=handles.hrtf_struct(handles.ind(handles.R_azs_num(handles.click_num))).az
guidata(hObject, handles);
...
handles.test_result(handles.test_num).HRTF_BASE=1;
